import { randomBool, randomDate, randomInt, dateToISOInt } from '../../test_data/util.js'
...
export const generateOffDay = async () => {
  ...
  for (const day of holidays) {
    offDays.push({ date: dateToISOInt(day.date), is_holiday: true, description: day.info })
  }
  for (const date of sundays) {
    offDays.push({ date: dateToISOInt(date), is_weekend: true })
  }
  await knex('off_days').insert(offDays)

  // Offdays for Saturday Off only till class 5
  const classesTill5 = await knex('_class').select('*').limit(5)
  for (const cls of classesTill5) {
    const saturdayOff = []
    for (const date of saturdays) {
      saturdayOff.push({ date: dateToISOInt(date), is_weekend: true, class_id: cls.id })
    }
    await knex('off_days').insert(saturdayOff)
  }
}
